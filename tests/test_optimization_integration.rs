...
fn test_end_to_end_quantization_with_optimization() {
    ...
    assert!(
        result.is_ok(),
        "End-to-end quantization with optimization should succeed"
    );
    ...
    assert!(
        metrics.optimization_converged || metrics.optimization_iterations == 20,
...
fn test_optimization_performance_overhead() {
    ...
    assert!(
        ...
        "Baseline quantization should succeed"
    );
    ...
    let mut quantizer_optimized =
        TimeAwareQuantizer::with_thermodynamic_config(4, config_optimized);
    ...
    assert!(
        result_optimized.is_ok(),
        "Optimized quantization should succeed"
    );
    ...
    assert!(
        time_optimized.as_secs_f64() < 1.0,
...
fn test_backward_compatibility_disabled_by_default() {
    ...
    assert!(
        !default_config.transition_optimization.enabled,
    ...
    assert!(
        result.is_ok(),
        "Quantization should work with default config"
    );
...
fn test_optimization_with_different_learning_rates() {
    ...
    for learning_rate in [0.001, 0.01, 0.1] {
        ...
        assert!(
            result.is_ok(),
            "Quantization should succeed with learning_rate={}",
            learning_rate
        );
        ...
        assert!(
            metrics.optimization_iterations > 0,
            "Optimization should run with learning_rate={}",
            learning_rate
        );
        assert!(
            metrics.final_loss.is_finite(),
            "Final loss should be finite with learning_rate={}",
            learning_rate
        );
    }
}
...
fn test_optimization_with_different_markov_weights() {
    ...
    for markov_weight in [0.0, 0.1, 0.5] {
        ...
        assert!(
            result.is_ok(),
            "Quantization should succeed with markov_weight={}",
            markov_weight
        );
        ...
        assert!(
            metrics.final_loss.is_finite(),
            "Final loss should be finite with markov_weight={}",
            markov_weight
        );
    }
}
...
fn test_optimization_convergence_behavior() {
    ...
    assert!(
        metrics.optimization_iterations <= 100,
        "Should not exceed max iterations"
    );

    println!(
        "Converged: {}, Iterations: {}, Final loss: {}",
        metrics.optimization_converged, metrics.optimization_iterations, metrics.final_loss
    );
}
...
fn test_optimization_with_both_beta_schedules() {
    ...
    for beta_schedule in [BetaSchedule::Linear, BetaSchedule::Cosine] {
        ...
        assert!(
            result.is_ok(),
            "Quantization should succeed with {:?} beta schedule",
            beta_schedule
        );
        ...
        assert!(
            metrics.optimization_iterations > 0,
            "Optimization should run with {:?} beta schedule",
            beta_schedule
        );
        assert!(
            metrics.final_loss.is_finite(),
            "Final loss should be finite with {:?} beta schedule",
            beta_schedule
        );
    }
}
...
fn test_optimization_with_larger_model() {
    ...
    assert!(
        result.is_ok(),
        "Optimization should work with larger models"
    );
