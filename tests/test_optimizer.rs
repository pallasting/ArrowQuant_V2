...
use arrow_quant_v2::thermodynamic::optimizer::{OptimizerConfig, TransitionOptimizer};
...
fn test_parameter_updates_modify_params() {
    ...
    let params_changed = result.params.iter().any(|p| {
        (p.scale - initial_scale).abs() > 1e-6 || (p.zero_point - initial_zero_point).abs() > 1e-6
    });

    assert!(
        params_changed,
        "Parameters should be modified during optimization"
    );
}
...
fn test_convergence_detection_early_stop() {
    ...
    if result.converged {
        ...
        if result.loss_history.len() >= 2 {
            ...
            let loss_change =
                (result.loss_history[last_idx - 1] - result.loss_history[last_idx]).abs();
            assert!(
                loss_change < 0.5,
                "Loss change should be below threshold: {}",
                loss_change
            );
        }
    }
...
fn test_early_stopping_returns_best_params() {
    ...
    assert!(
        (result.final_loss - min_loss).abs() < 1e-5,
...
fn test_gradient_clipping() {
    ...
    for (initial, final_param) in initial_params.iter().zip(result.params.iter()) {
        ...
        assert!(
            scale_change < 1.0,
            "Scale change should be bounded by gradient clipping: {}",
            scale_change
        );
        assert!(
            zp_change < 1.0,
            "Zero point change should be bounded by gradient clipping: {}",
            zp_change
        );
    }
}
...
fn test_different_beta_schedules() {
    ...
    for schedule in [BetaSchedule::Linear, BetaSchedule::Cosine] {
        ...
        assert!(
            result.is_ok(),
            "Optimization should succeed with {:?} schedule",
            schedule
        );
...
fn test_loss_history_tracking() {
    ...
    for (i, &loss) in result.loss_history.iter().enumerate() {
        ...
        assert!(
            loss >= 0.0,
            "Loss at iteration {} should be non-negative",
            i
        );
    }
}
...
fn test_parallel_optimization() {
    ...
    let results = optimizer
        .optimize_params_parallel(&layers, &initial_params)
        .unwrap();
...
fn test_zero_learning_rate() {
    ...
    for (initial, final_param) in initial_params.iter().zip(result.params.iter()) {
        assert!(
            (final_param.scale - initial.scale).abs() < 1e-6,
            "Scale should not change with zero learning rate"
        );
        assert!(
            (final_param.zero_point - initial.zero_point).abs() < 1e-6,
