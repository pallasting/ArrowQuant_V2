...
fn test_validation_with_realistic_diffusion_params() {
    ...
    assert!(
        result.is_ok(),
        "Quantization with realistic params should succeed"
    );
...
fn test_validation_with_large_parameter_jumps() {
    ...
    assert!(
        result.is_ok(),
        "Quantization should succeed even with violations"
    );
...
fn test_validation_preserves_quantization_accuracy() {
    ...
    let error_with_validation =
        compute_reconstruction_error(&weights, &result_with_validation, &params);
...
fn test_validation_performance_scales_linearly() {
    ...
    for num_groups in [5, 10, 20] {
        ...
        println!(
            "Performance with {} groups: overhead = {:.2}%",
            num_groups, overhead_percent
        );
...
fn test_validation_with_edge_case_parameters() {
    ...
    let quantizer_single =
        TimeAwareQuantizer::with_thermodynamic_config(1, thermodynamic_config.clone());
    let params_single = vec![TimeGroupParams {
        time_range: (0, 1000),
        scale: 0.05,
        zero_point: 128.0,
        group_size: 256,
    ...
    let quantizer_small =
        TimeAwareQuantizer::with_thermodynamic_config(2, thermodynamic_config.clone());
...
fn test_validation_logging_configuration() {
    ...
    let quantizer_no_logging = TimeAwareQuantizer::with_thermodynamic_config(2, config_no_logging);
