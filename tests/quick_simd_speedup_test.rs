...
use arrow_quant_v2::simd::{is_simd_available, quantize_simd};
...
fn test_simd_speedup_1k() {
    ...
    assert!(
        speedup >= 2.0,
        "Expected at least 2x speedup for 1K array, got {:.2}x",
        speedup
    );
}
...
fn test_simd_speedup_10k() {
    ...
    assert!(
        speedup >= 2.5,
        "Expected at least 2.5x speedup for 10K array, got {:.2}x",
        speedup
    );
}
...
fn test_simd_speedup_100k() {
    ...
    assert!(
        speedup >= 3.0,
        "Expected at least 3x speedup for 100K array, got {:.2}x",
        speedup
    );
}
...
fn test_simd_speedup_1m() {
    ...
    assert!(
        speedup >= 3.0,
        "Expected at least 3x speedup for 1M array, got {:.2}x",
        speedup
    );
}
...
fn test_simd_availability() {
    ...
    if simd_width.is_available() {
        println!(
            "  ✅ SIMD is available on this platform: {:?} (width: {})",
            simd_width,
            simd_width.width()
        );
    } else {
