...
impl EvolutionarySearch {
    ...
    pub fn search(
        ...
        log::info!(
            "Starting evolutionary search with {} generations, population size {}",
        ...
        for generation in 0..self.config.num_generations {
            ...
            if self.evaluations >= self.config.max_evaluations {
                log::warn!(
                    "Reached maximum evaluation budget: {}",
            ...
            if let Some(current_best) = population.first() {
                if best_individual.is_none()
                    || current_best.fitness > best_individual.as_ref().unwrap().fitness
                {
            ...
            let gen_stats = GenerationStats {
                ...
                avg_fitness: population.iter().map(|i| i.fitness).sum::<f32>()
        ...
        log::info!(
            "Evolutionary search complete. Best fitness: {:.4}",
            best.fitness
        );
    ...
    fn evaluate_population(
        ...
        for (idx, individual) in population.iter_mut().enumerate() {
            ...
            log::debug!(
                "Evaluating individual {}/{} in generation {}",
                idx + 1,
                pop_len,
                generation
            );
            ...
            match DiffusionOrchestrator::new(config) {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to create orchestrator for individual {}: {}",
                        idx,
                        e
                    );
                    individual.fitness = 0.0;
