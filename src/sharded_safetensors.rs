...
impl ShardedSafeTensorsAdapter {
    ...
    pub fn load<P: AsRef<Path>>(index_path: P) -> Result<Self> {
        ...
        let index_content = std::fs::read_to_string(index_path)
            .map_err(|e| QuantError::Storage(format!("Failed to read index file: {}", e)))?;

        let index: ShardedIndex = serde_json::from_str(&index_content)
            .map_err(|e| QuantError::Storage(format!("Failed to parse index JSON: {}", e)))?;
...
pub fn find_index_file<P: AsRef<Path>>(dir: P) -> Result<PathBuf> {
    ...
    for entry in std::fs::read_dir(dir)
        .map_err(|e| QuantError::Storage(format!("Failed to read directory: {}", e)))?
    {
        let entry =
            entry.map_err(|e| QuantError::Storage(format!("Failed to read entry: {}", e)))?;
